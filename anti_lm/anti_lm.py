...
def construct_lm_softmax(sess, model, validation_input_lengths):
    ...
    for dummy_batch, dummy_batch_lens in tqdm(
        ...
        print("dummmy batch lens: {}".format(dummy_batch_lens))
        decoder_softmax = sess.run(model.dummy_decoder_softmax, feed_dict)

        print("1. {}".format(decoder_softmax.shape))
        print("2. {}".format((conf.anti_lm_max_penalization_len, len(dummy_batch_lens), conf.vocabulary_size)))
        assert decoder_softmax.shape == (conf.anti_lm_max_penalization_len, len(dummy_batch_lens), conf.vocabulary_size)

        all_softmax.append(decoder_softmax)

    all_softmax = np.vstack(all_softmax)
    print("3. {}".format(all_softmax.shape))
    print("4. {}".format((len(sorted_padding_lengths), conf.anti_lm_max_penalization_len, conf.vocabulary_size)))
    assert all_softmax.shape == (len(sorted_padding_lengths), conf.anti_lm_max_penalization_len, conf.vocabulary_size)
...
def construct_lm_softmax_batch(all_softmax_dict, batch_sequence_lengths):
    ...
    assert batch_output_softmax.shape == (len(batch_sequence_lengths), conf.anti_lm_max_penalization_len, conf.vocabulary_size)
